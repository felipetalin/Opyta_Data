...
# --- CONFIGURAÇÕES ---
ARQUIVO_EXCEL = sys.argv[1] if len(sys.argv) > 1 else "projeto_avifauna_real.xlsx"
GRUPO_BIOLOGICO_ALVO = sys.argv[2] if len(sys.argv) > 2 else "Avifauna"
...
def validar_abas_excel(xls: pd.ExcelFile, abas_obrigatorias: list[str]) -> None:
    """
    Valida se todas as abas obrigatórias existem no Excel.
    """
    abas_ausentes = [aba for aba in abas_obrigatorias if aba not in xls.sheet_names]
    if abas_ausentes:
        raise ValueError(
            f"Abas obrigatórias ausentes no Excel: {abas_ausentes}. "
            f"Abas disponíveis: {xls.sheet_names}"
        )
    logger.info(f"Validação de abas OK: {abas_obrigatorias}")
...
def normalizar_data(valor):
    """
    Normaliza data para datetime, retornando None se inválida.
    """
    if pd.isna(valor):
        return None
    try:
...
def limpar_dados_da_campanha(connection, id_projeto, df_pontos_da_planilha, tabela_resultados: str):
    """
    Limpa APENAS os dados do grupo biológico das campanhas presentes na planilha.
    NÃO apaga pontos de coleta, pois eles podem ser usados por outros grupos.
    """
    ...
    if not ids_campanha_para_limpar:
        logger.info("Campanhas são novas no banco, nenhuma limpeza necessária.")
        return
    ...
    logger.info(f"Removendo esforços antigos de {GRUPO_BIOLOGICO_ALVO}...")
    ...
    logger.info(f"Pontos de coleta preservados (infraestrutura compartilhada).")
    logger.info(f"Limpeza de {GRUPO_BIOLOGICO_ALVO} concluída com segurança.")
...
def migrar_dados(connection, df_capa, df_pontos, df_esforco, df_resultados, tabela_resultados: str):
    ...
    for coluna in colunas_obrigatorias_esforco:
        if coluna not in df_esforco.columns:
            raise ValueError(
                f"A aba 'Metadados_Esforco' não possui a coluna obrigatória '{coluna}'."
            )
    ...
    for coluna in colunas_obrigatorias_resultados:
        if coluna not in df_resultados.columns:
            raise ValueError(
                f"A aba '{NOME_ABA_RESULTADOS}' não possui a coluna obrigatória '{coluna}'."
            )

    logger.info("Colunas obrigatórias validadas em todas as abas.")
    ...
    logger.info(f"Processando Esforços de Amostragem de {GRUPO_BIOLOGICO_ALVO}...")
    pontos_db_map = (
        pd.read_sql(
            text(
                """
    ...
    logger.info(
        f"{len(esforcos_records)} esforços de {GRUPO_BIOLOGICO_ALVO} "
    ...
    # Remover validação duplicada (já feita no início da função)
    ...
    possui_observacoes = "Observacoes" in df_resultados.columns
    ...
    if resultados_records:
        # SEGURO: Use parameterização, não f-string para nome da tabela
        # Se a tabela não existe, o banco retornará erro, não SQL injection
    ...
    if warnings_especies:
        logger.warning(
            f"Os seguintes táxons não foram encontrados no cadastro mestre: "
            f"{sorted(list(warnings_especies))}"
        )

    if warnings_esforcos:
        logger.warning(
            f"{warnings_esforcos} linha(s) de resultado não puderam ser "
            "mapeadas para esforço/ponto/método."
        )

    if possui_observacoes:
        logger.info("Coluna 'Observacoes' foi detectada, mas não foi agregada por táxon nesta versão.")


def main():
    if not os.path.exists(ARQUIVO_EXCEL):
        logger.error(f"Arquivo '{ARQUIVO_EXCEL}' não foi encontrado.")
        sys.exit(1)

    engine = None

    try:
        logger.info(f"Iniciando migração de {GRUPO_BIOLOGICO_ALVO}: {ARQUIVO_EXCEL}")
        ...
        logger.info(f"Excel carregado com sucesso: {len(df_pontos)} pontos, {len(df_resultados)} resultados")

        # Normalizar datas usando função auxiliar
        if "Data" in df_pontos.columns:
            df_pontos["Data"] = df_pontos["Data"].apply(normalizar_data)
            logger.info("Datas normalizadas com sucesso")
        ...
        with engine.begin() as connection:
            ...
            id_projeto = connection.execute(
                text(
                    ...
                    SELECT id_projeto
                    FROM projetos
                    WHERE codigo_interno_opyta = :Codigo_Opyta
                    """
                ),
                params_projeto,
            ).scalar_one()
            ...
            logger.info(f"✓ MIGRAÇÃO DE {GRUPO_BIOLOGICO_ALVO.upper()} CONCLUÍDA COM SUCESSO")

    except Exception as e:
        logger.error(f"ERRO DURANTE A MIGRAÇÃO. A TRANSAÇÃO FOI REVERTIDA (ROLLBACK)")
        ...
        if engine is not None:
            engine.dispose()
            logger.info("Recursos de conexão liberados.")
