# --- SCRIPT DE MIGRAÇÃO - ZOOBENTOS (V5.2 com Limpeza Segura) ---
...
def migrar_dados(connection, id_projeto, df_capa, df_pontos, df_esforco, df_resultados):
    ...
    df_resultados['Abundancia'] = df_resultados.get('Abundancia') if 'Abundancia' in df_resultados else df_resultados.get('Numero_de_Individuos')
    df_resultados_agregado = df_resultados.groupby(['Campanha', 'Ponto', 'Metodo_de_Captura', 'Nome_Cientifico', 'Tipo_de_Amostragem']).agg(Abundancia=('Abundancia', 'sum')).reset_index()
    ...
    for _, row in df_resultados_agregado.iterrows():
        ...
        if id_esforco and id_especie:
            resultados_records.append({"id_esforco": id_esforco, "id_especie": id_especie, "abundancia": row.get('Abundancia'), "tipo_amostragem": row.get('Tipo_de_Amostragem')})
        elif not id_especie: warnings_especies.add(nome_cientifico_clean)
    
    if resultados_records:
        query_resultados = text(f"INSERT INTO {NOME_TABELA_RESULTADOS} (id_esforco, id_especie, abundancia, tipo_amostragem) VALUES (:id_esforco, :id_especie, :abundancia, :tipo_amostragem) ON CONFLICT (id_esforco, id_especie) DO UPDATE SET abundancia = EXCLUDED.abundancia, tipo_amostragem = EXCLUDED.tipo_amostragem")
