# --- SCRIPT DE MIGRAÇÃO - ZOOPLÂNCTON (V5.2 com Limpeza Segura) ---
...
# --- CONFIGURAÇÕES ---
ARQUIVO_EXCEL = sys.argv[1] if len(sys.argv) > 1 else 'projeto_zoo_real.xlsx'
GRUPO_BIOLOGICO_ALVO = 'Zooplâncton'
NOME_TABELA_RESULTADOS = 'resultados_zooplancton'
NOME_ABA_RESULTADOS = 'Resultados_Zooplancton'
...
def migrar_dados(connection, id_projeto, df_capa, df_pontos, df_esforco, df_resultados):
    ...
    # Para Zooplâncton, os dados geralmente já são agregados, então não precisamos da etapa de 'groupby'
    ...
    for _, row in df_resultados.iterrows():
        ...
        if id_esforco and id_especie:
            resultados_records.append({"id_esforco": id_esforco, "id_especie": id_especie, "numero_de_individuos": row.get('Numero_de_Individuos'), "unidade_contagem": row.get('Unidade_Contagem'), "tipo_amostragem": row.get('Tipo_de_Amostragem')})
        elif not id_especie: warnings_especies.add(nome_cientifico_clean)

    if resultados_records:
        # Usamos ON CONFLICT aqui como uma dupla garantia
        query_resultados = text(f"INSERT INTO {NOME_TABELA_RESULTADOS} (id_esforco, id_especie, numero_de_individuos, unidade_contagem, tipo_amostragem) VALUES (:id_esforco, :id_especie, :numero_de_individuos, :unidade_contagem, :tipo_amostragem) ON CONFLICT (id_esforco, id_especie) DO UPDATE SET numero_de_individuos = EXCLUDED.numero_de_individuos, unidade_contagem = EXCLUDED.unidade_contagem, tipo_amostragem = EXCLUDED.tipo_amostragem")
